...
- 待归档文件不进入正式项目 Context；归档成功后才写入事实与生成新 Context 版本。删除单个归档文件时从已验证项目快照移除对应事实并将 Context 标记为待刷新，避免已删除文件重新进入项目记忆；
...
- 项目记忆已由追加式历史对象改为每项目稳定 `snapshot.json`，并通过 `revision.json` 轻量校验进程缓存；版本未变化不下载完整快照，旧历史只在新快照回读验证后清理。新增/移动/删除路径记录在 Context 生命周期变更清单中，供下一次重建优先选取或排除证据；
- 分类和归档响应已返回分阶段耗时与模型调用体量，页面诊断面板可直接识别耗时来自文件解析、事实抽取、记忆加载、S3 归档还是 Context 综合；事实抽取限制为 2048 tokens，扫描 PDF 单批 OCR 限制为 1200 tokens；
- 已实现正式 Context 生命周期：候选判断只读取最新已提交 Context；归档成功后提交文件事实并生成新版本；移动、删除只标记 `dirty`，不立即调用模型；用户可手动刷新，或者由下一次上传在判断前自动刷新；
...
补充进度：当前线上上传链路使用 S3 项目快照，不再要求前端显式传入之前上传文件的事实。稳定的 `snapshot.json` 保存当前有效状态，`revision.json` 用于低成本判断进程缓存是否仍为最新版；旧追加式数据在快照验证成功后安全压缩。它支持 Coze 服务重启、重新部署和多实例切换后的恢复；S3 暂时不可用时才降级为有时限的进程缓存。结构化查询、权限、审核状态和统计仍留给最终 Supabase 项目记忆库。

Context 生命周期补充：系统按每次上传一份文件处理，不引入 batch 或批次完成信号。新上传文件在正式归档前只是候选，不写入项目 Context；本次 Agent 建议会记录实际使用的 Context 版本。归档成功后提交该文件事实并生成新版本。`dirty` 表示已有归档文件移动或删除后，旧 Context 已过期；这类操作只保存变更清单、不立即调用 LLM，左栏可手动刷新，否则下一份文件分析前自动刷新。
