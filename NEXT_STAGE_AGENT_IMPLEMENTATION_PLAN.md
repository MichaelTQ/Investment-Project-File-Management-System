...
- 已实现 `project-context-synthesizer-v2`：使用当前全部已归档、有效的 `DocumentFacts` 事实卡片重建项目事件时间线、阶段假设、文件关系、冲突和待确认问题；正常线上请求调用一次 Coze LLM，模型无 JSON、格式错误或请求异常时自动生成确定性规则 Context，并把原因记录为数据质量提示而非更新失败；
