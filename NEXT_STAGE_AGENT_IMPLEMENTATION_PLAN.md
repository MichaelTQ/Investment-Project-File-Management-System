...
- 分类和归档响应已返回顶层/子阶段耗时与模型调用体量，页面诊断面板可区分文件 Buffer、签名 URL、本地 PDF、FetchClient、OCR、事实抽取、记忆加载、S3 归档和 Context 综合；流式模型记录响应头、后续生成和完整耗时。事实抽取限制为 1200 tokens，扫描 PDF 单批 OCR 限制为 1200 tokens；
