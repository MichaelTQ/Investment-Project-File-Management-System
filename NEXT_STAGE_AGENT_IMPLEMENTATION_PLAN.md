...
- 已实现 `project-context-synthesizer-v1`：使用当前全部已归档、有效的 `DocumentFacts` 事实卡片重建项目事件时间线、阶段假设、文件关系、冲突和待确认问题；正常线上请求调用一次 Coze LLM，失败时保留上一版 Context 并标记失败；
...
- `context-decision-v3` 在原有试点规则外增加通用文档类型到目录候选的阶段校验，可处理增资协议、股东协议、投委会决议、银行回单、立项材料、商业计划书、尽调报告等已建模类型；证据不足或同名跨阶段冲突时仍转人工；
