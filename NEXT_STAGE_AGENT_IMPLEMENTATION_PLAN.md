...
- 已实现 `project-context-synthesizer-v1`：项目文件新增、替换或删除后，使用当前全部有效 `DocumentFacts` 事实卡片重建项目事件时间线、阶段假设、文件关系、冲突和待确认问题；正常线上请求调用一次 Coze LLM，失败时生成带警告的确定性降级快照；
- 项目上下文综合器不使用上传顺序推断业务阶段；模型生成的事件和关系必须引用当前项目中真实存在的 `sourcePath`，无来源事件会被丢弃；删除文件后会重新生成快照，避免已删除证据残留；
