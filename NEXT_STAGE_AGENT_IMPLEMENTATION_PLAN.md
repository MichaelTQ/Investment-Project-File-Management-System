...
- 待归档文件不进入正式项目 Context；归档成功后才写入事实与生成新 Context 版本。删除单个归档文件时写入 S3 tombstone 并将 Context 标记为待刷新，避免已删除文件重新进入项目记忆；
...
- 已实现 `project-context-synthesizer-v4`：使用当前全部已归档、有效的 `DocumentFacts` 事实卡片重建项目事件时间线、阶段假设、文件关系、冲突和待确认问题；直接调用 Chat Completions，显式设置 8192 输出 tokens 和 JSON 模式，并保留 `finish_reason` 与输出 token 用量。输出数量和字段长度均有限制，首次响应发生 JSON 截断或 Schema 错误时自动使用紧凑模式重试一次，两次都失败才生成确定性规则 Context；
...
补充进度：当前线上上传链路已加入 S3 追加式项目记忆，不再要求前端显式传入之前上传文件的事实。它支持 Coze 服务重启、重新部署和多实例切换后的恢复；S3 暂时不可用时才降级为有时限的进程缓存。S3 方案解决开发阶段的连续记忆，但结构化查询、权限、审核状态和统计仍留给最终 Supabase 项目记忆库。

Context 生命周期补充：新上传文件在正式归档前只是候选，不写入项目 Context；本次 Agent 建议会记录实际使用的 Context 版本。归档成功后才提交事实并生成新版本。移动和删除仅写 tombstone/脏状态，不立即调用 LLM；左栏项目 Context 面板可手动刷新，若未手动刷新，下一份新文件判断前会自动刷新。
