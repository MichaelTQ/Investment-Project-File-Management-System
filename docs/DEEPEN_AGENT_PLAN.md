...
1. ~~抽出 `read-document-content.ts`，主链路替换，跑通现有测试~~ ✅ **已完成 2026-08-09**。
   `/api/classify` 从 1038 行降到 776 行；阶段名、OCR 提示词、常量、模型 ID、回退顺序
   均逐字比对确认未变；类型检查、lint、137 个测试全过。深挖的 `extract_document_facts`
   工具现在可以调用与主链路完全相同的读取链路——两边读到的必须是同一个东西，
   否则第 8 节的评测失去意义。
...
3. ~~补齐其余四个工具 + 预算控制 + 轨迹记录~~ ✅ **已完成 2026-08-09**。
   `src/lib/classification/deepen/{types,tools,agent}.ts`，五个工具全部就位，
   循环 + 预算 + 轨迹 + 五种停止原因。8 个行为测试用注入的假网关/假档案/假判定器
   覆盖循环本身（`DeepenDeps` 接缝）。**尚未接入任何接口和界面**，只能从代码调用。
4. ~~接 `/api/deepen`，前端加按钮~~ ✅ **已完成 2026-08-09**。
   `POST /api/deepen`（未启用时返回 403）；界面上在文件行右键菜单里加「深挖：自动查证再判」，
   结果面板显示停止原因、取证结论、归档建议和可展开的完整执行轨迹。
   **轨迹是给用户看的，不是调试信息**——系统凭什么这么判全在里面，看不见就只能一路点确认。
