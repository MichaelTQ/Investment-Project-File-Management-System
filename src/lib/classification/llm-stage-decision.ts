...
先看这几个候选是否与文件内容相符。**若要选候选之外的阶段，必须能引用文件原文说明为什么**，并在理由里写清与规范提示不同的依据。
【说不清就输出 unknown】拿不出这样的原文依据时，不要在候选之外随手挑一个看起来接近的阶段——输出 unknown 交人工判断。宁可多一些交人工的，也不要给出一个说不出依据的答案。`
    : '';
