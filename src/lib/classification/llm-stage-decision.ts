...
7. 【拿不准就输出 unknown】事实不足以判断时输出 unknown，并在 why 里说明缺什么。只有当你能引用具体原文说明"它记录的是哪一步业务动作"时才给出阶段；只是"看起来像"或"通常属于"都不够。交人工的成本远低于放错位置的成本，宁可多输出 unknown。
...
**你只有两种选择：从上面的候选里选一个，或者输出 unknown。**
先看这几个候选哪一个与文件内容相符；如果文件内容与所有候选都对不上，输出 unknown 交人工判断，并在 why 里说明它看起来更像什么。
【不要选候选之外的阶段】规范没覆盖到的情况确实存在，但那种时候正确的做法是交给人，而不是自己另挑一个——一份被自信地放错位置的文件，比一份等人处理的文件代价大得多。`
    : '';
