...
4. 【数值比对必须发生在两份不同的文件之间】同一份文件里同时出现变更前值和变更后值
   ...
   但要接着往下推一步，而且这一步最容易推错：**形成于变更之前，不等于应当归入更早
   的阶段。** 促成这次变更的那些文件本来就形成于变更之前，正文里记的当然是变更前的
   值；它们和记载变更的那份文件说的是同一件事，归在同一个阶段是正常形态，不是矛盾。
   一个阶段横跨变更两侧，恰恰是这个阶段正在做这件事的样子。
   分两种情况：
   （a）吻合变更**前**值的文件，归在**比记载变更的那份更晚**的阶段——时点和归档
   位置真正对不上，必须报出来。变更完成之后不会再产生记录变更前状态的文件。
   （b）两份在**同一个阶段**——先判断这份文件是不是促成这次变更的一方：它记载的
   主体里如果已经出现了这次变更才新引入的主体，它就属于这件事本身，不要报；只有当
   它单纯描述变更之前的状态、且与新引入的主体无关时，才值得报出来请人确认。
   判断不了属于哪种，就不要报。
