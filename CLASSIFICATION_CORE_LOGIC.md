...
文档事实抽取使用短字段、数组元组协议和最小事实集合：`max_tokens=600`、120 秒超时，目标实际输出 250～450 tokens，最多保留日期 2 项、主体 4 项、交易变化 3 项、阶段线索/引文各 2 项，并禁止同一证据在多个字段重复；服务端会将紧凑响应还原为稳定的完整 `DocumentFacts` Schema。相同项目、相同内容指纹的成功抽取结果可在进程内复用 12 小时，缓存命中时同时跳过远端文件解析和 LLM；失败降级结果不缓存。带文字层且不超过 25MB 的 PDF 优先用 PDF.js 在本地提取（最多 100 页/12,000 字符），扫描件、本地解析失败或超限文件自动回退到 FetchClient/OCR；扫描 PDF 单批 OCR 上限为 1200 tokens。

页面默认开启规则检查。文件进入上传流程后先按文件名运行确定性阶段规则并立即显示“快速预判”；该结果不开放归档确认，Agent 使用正文事实和项目 Context 完成后在同一结果卡片中替换为完整双轨结果。

人工确认归档后，请求只等待业务文件入库和带 `dirty` 状态的正式事实快照持久化，不再同步等待 Context LLM。页面收到归档成功后独立调用 Context 重建接口；后台更新失败时已归档文件和事实不会丢失，后续可手动更新或在下一次候选判断前自动重建。分类 API（`/api/classify`）同样使用 `deferContextRebuild: true` 延迟重建，返回 `contextRebuildPending` 标志，前端收到后异步触发 Context 重建。

Context 重建后的文件重评估采用增量策略：加文件触发的重建只重评估受影响文档（被新 Context 引用、本轮变更、同文档类型或同当事方）；删除文件或手动重建时自动升级为全量重评估。增量策略将 50 份文件项目从 50 次 Agent 调用缩减到通常 2-5 次。

性能诊断区分顶层阶段与文件读取、签名 URL、FetchClient、本地 PDF、OCR 等子阶段。流式 LLM 同时显示响应头耗时、后续生成耗时和完整耗时，不再把收到响应头误报为调用完成。

项目记忆使用每项目逻辑 `snapshot` 保存当前有效事实和 Context，并用很小的 `revision` 对象校验进程缓存。Coze S3 会给上传 key 自动增加随机后缀，因此系统始终使用上传返回的真实 key 回读校验，并在成功后清理本次写入前已经确认的旧快照；不会按不存在的逻辑文件名回读，也不会误删并发实例刚写入的新对象。版本未变化时无需重新下载完整快照；旧的追加式历史对象只有在新快照写入并回读验证成功后才会清理。删除归档文件时先删除对应 S3 业务对象，失败则保留数据库记录并报错，避免留下无法追踪的垃圾对象；删除成功后再从项目快照移除事实，且不会误删同项目其他有效证据。
