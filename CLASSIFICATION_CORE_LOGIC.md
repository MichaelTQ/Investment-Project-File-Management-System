...
文档事实抽取使用最小事实集合：`max_tokens=1200`、120 秒超时，最多保留日期 4 项、主体 8 项、交易变化 6 项、阶段线索/引文各 4 项，并禁止同一证据在多个字段重复。相同项目、相同内容指纹的成功抽取结果可在进程内复用 12 小时，缓存命中时同时跳过远端文件解析和 LLM；失败降级结果不缓存。带文字层且不超过 25MB 的 PDF 优先用 PDF.js 在本地提取（最多 100 页/12,000 字符），扫描件、本地解析失败或超限文件自动回退到 FetchClient/OCR；扫描 PDF 单批 OCR 上限为 1200 tokens。
